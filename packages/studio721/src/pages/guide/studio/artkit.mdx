...
- 🤯<SpacerHorizontal inline size={20} />Create _large_ NFT collections (up to
  20,000)
...
you're really creating a new zip file. You can save or load a collection into
...
- **Create Collection**
- **Create Generative Art Collection** - Use this if you want to your NFT to be
  an interactive webpage and/or if you want to generate your images previews or
...
tool duplicates a token file of your choosing (or a blank file, if you uncheck
...
You can select multiple by holding Shift or Cmd/Ctrl. There's a shortcut for
selecting all of your token files: right click any of the `.token.json` files
and choose "Select Sibling Files". This is the fastest way to edit all of your
tokens at once.

### Generating Attributes and Metadata

The "generative art collection" template includes a sample html/css/js setup for
optionally generating metadata attributes and/or image previews.
...
& Attributes..." at the top of the page. This tool automatically generates
images and attributes for each `.token.json` file in your collection zip.

The way it works: Artkit bundles your code files into a single html string and
load it into an `<iframe>`. Artkit then waits for your code to call
...
2. Next, all of your `/metadata` files are bundled into a "Car" file and
   ...
   the file uploaded in the previous step.
